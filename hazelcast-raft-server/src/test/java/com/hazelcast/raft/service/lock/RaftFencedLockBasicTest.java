...
public class RaftFencedLockBasicTest extends HazelcastRaftTestSupport {
    ...
    private RaftFencedLockProxy createLock(String name) {
        ...
        try {
            ...
            return new RaftFencedLockProxy(name, groupId, sessionManager, raftService.getInvocationManager());
        } catch (Exception e) {
    ...
    public void test_ReentrantLockFails_whenSessionClosed() throws ExecutionException, InterruptedException {
        ...
        NodeEngineImpl nodeEngine = getNodeEngineImpl(lockInstance);
        final SessionManagerService sessionManager = nodeEngine.getService(SessionManagerService.SERVICE_NAME);
        ...
        RaftService raftService = nodeEngine.getService(RaftService.SERVICE_NAME);
        raftService.getInvocationManager().invoke(groupId, new CloseSessionOp(sessionId)).get();
        assertTrueEventually(new AssertTask() {
            @Override
            public void run() {
    ...
    private void lockByOtherThread(final RaftFencedLockProxy lock) {
        try {
            spawn(new Runnable() {
                @Override
                public void run() {
