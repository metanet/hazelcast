...
public class AppendRequestHandlerTask extends RaftNodeAwareTask implements Runnable {
        ...
        if (req.entryCount() > 0) {
            ...
            for (int i = 0; i < req.entryCount(); i++) {
                ...
                if (reqEntry.term() != localEntry.term()) {
                    ...
                    handleRaftCommand(truncated, true);
            ...
            if (newEntries != null && newEntries.length > 0) {
                ...
                handleRaftCommand(asList(newEntries), false);
            }
        }
    ...
    private void handleRaftCommand(List<LogEntry> entries, boolean revert) {
        for (LogEntry entry : entries) {
            if (entry.operation() instanceof TerminateRaftGroupCommand) {
                RaftNodeStatus status = revert ? RaftNodeStatus.ACTIVE : RaftNodeStatus.TERMINATING;
                raftNode.setStatus(status);
                return;
            } else if (entry.operation() instanceof ApplyRaftGroupMembersCommand) {
                RaftNodeStatus status = revert ? RaftNodeStatus.ACTIVE : RaftNodeStatus.CHANGING_MEMBERSHIP;
                raftNode.setStatus(status);
                if (revert) {
                    raftNode.resetGroupMembers();
                } else {
                    ApplyRaftGroupMembersCommand op = (ApplyRaftGroupMembersCommand) entry.operation();
