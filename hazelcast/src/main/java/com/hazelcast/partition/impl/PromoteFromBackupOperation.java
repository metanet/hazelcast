...
final class PromoteFromBackupOperation extends AbstractOperation
    ...
    private void setMissingReplicaVersions() {
        ...
        if (loggable) {
            log = "Setting missing replica versions for partitionId=" + getPartitionId() + " overwriting-replicaIndex=" + (ix + 1)
                    + " Changed from replicaVersions=" + Arrays.toString(versions);
        }
