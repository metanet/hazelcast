...
public abstract class AbstractJoiner implements Joiner {
    ...
    /**
     * Sends a split brain join request to the target address, and returns the request to the caller
     * if this node should merge to the target address
     */
    protected final SplitBrainJoinMessage sendAndCheckSplitBrainJoinMessage(Address target) {
        ...
        return (clusterService.shouldMerge(response) == THIS_NODE_SHOULD_MERGE) ? request : null;
    }
